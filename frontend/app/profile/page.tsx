                ...
                <TabsContent value="improver" className="space-y-6">
                  ...
                  <div>
                    <div className="flex items-center gap-3 mb-4">
                      <div className="text-lg font-semibold">ATS Compatibility</div>
                      <Badge variant={data.ats_score && data.ats_score > 70 ? "default" : "destructive"} className="text-base px-3 py-1">
                    ...
                    {data.ats_warnings && data.ats_warnings.length > 0 ? (
                      <div className="border rounded-lg p-4 bg-muted/30">
                        <div className="font-medium mb-2 text-destructive">Issues to Fix</div>
                        <ul className="list-disc pl-5 space-y-1 text-sm">
                          {data.ats_warnings.map((w, i) => <li key={i}>{w}</li>)}
                        </ul>
                      </div>
                    ) : (
                      <div className="text-sm text-muted-foreground italic">No major ATS issues found.</div>
                    )}
                  </div>
                  ...
                  <div>
                    <div className="text-lg font-semibold mb-1">Impact Rewrites (STAR Method)</div>
                    <p className="text-sm text-muted-foreground mb-4">
                      Transform weak bullet points into high-impact "Situation-Task-Action-Result" statements.
                    </p>

                    <div className="grid gap-4">
                      {data.star_rewrites?.map((r, idx) => (
                        <Card key={idx} className="border-l-4 border-l-primary/50">
                          <CardHeader className="pb-2">
                            ...
                            <div className="text-sm border-l-2 pl-3 py-1 italic opacity-80">{r.original}</div>
                          </CardHeader>
                          <CardContent className="space-y-3">
                            <div>
                              <div className="text-sm font-medium text-primary mb-1">Rewritten (STAR)</div>
                              <div className="text-sm font-medium">{r.rewritten}</div>
                            </div>
                            <div className="bg-muted p-2 rounded text-xs text-muted-foreground">{r.reasoning}</div>
                          </CardContent>
                        </Card>
                      ))}
                      {!data.star_rewrites?.length && (
                        <p className="text-sm text-muted-foreground">No rewrite suggestions available.</p>
                      )}
                    </div>
                  </div>
                </TabsContent>
